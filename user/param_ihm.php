...
if ($action == 'edit') {
	...
	print empty($bespoerp_main_demo) ? '' : ' disabled="disabled"'; // Disabled for demo
	print '> <label for="check_MAIN_LANG_DEFAULT">'.$langs->trans("UsePersonalValue").'</label></td>';
	print '<td>';
	print $formadmin->select_language((!empty($object->conf->MAIN_LANG_DEFAULT) ? $object->conf->MAIN_LANG_DEFAULT : ''), 'main_lang_default', 1, null, 0, 0, (!empty($bespoerp_main_demo)));
	print '</td></tr>';
	...
	// Theme
	showSkins($object, (($user->admin || empty($bespoerp_main_demo)) ? 1 : 0), true);
	...
	if (empty($user->admin) && !empty($bespoerp_main_demo)) {
