...
dol_include_once("/moceanapi/core/class/moceanapi_sms_reminder.db.php");
...
class SMS_Invoice_Setting extends MoceanBaseSettingController
{
	...
	public $db_key = "MOCEANAPI_INVOICE_SETTING";
	...
	private function get_default_settings()
	{
		$settings = array(
			...
			"sms_templates" => array(
				"created" 	=> "Hi [company_name], we are drafting your invoice now and will send to you once it's ready.",
				...
				"paid"      => "Hi [company_name], thank you for your payment, your invoice ([ref]) is paid.",
			),
		);
	...
	public function update_settings()
	{
		...
		if(!empty($_POST)) {
			...
			if ($action == "update_{$this->context}") {
				...
				if ($error < 1) {
					$this->log->add("MoceanAPI", "failed to update the invoice settings: " . print_r($settings, 1));
	...
	public function get_settings()
	{
		...
		if (empty($settings)) {
			...
			if(!array_key_exists("reminder_settings", $decoded_setting)) {
				$decoded_setting->reminder_settings = array();
	...
	public function bill_trigger_sms(Facture $object, $action)
	{
		...
		if($action === 'validated') {
			try {
				...
				$this->log->add("MoceanAPI", "Error occured when scheduling sms reminder");
				$this->log->add("MoceanAPI", $e->getTraceAsString());
			}
		}
		...
		$this->log->add("MoceanAPI", "current action triggered: {$action}");
		...
		$this->log->add("MoceanAPI", "SMS Successfully sent from fx bill_trigger_sms");
	}
	...
	public function schedule_sms_reminders(Facture $object)
	{
		...
		global $db;
		$this->log->add("MoceanAPI", "Scheduling SMS Reminders for object_id: {$object->id}");

		// invoice due date
		$invoice_overdue_date = $object->date_lim_reglement;
		$today = new DateTime("now", new DateTimeZone(getServerTimeZoneString()));
		// we dont reset the time so that we don't send dulpicate SMS
		// Situation: They add / delete reminder and save, when we reschedule, we will check if the
		// scheduled datetime is < now(include hour,minute,seconds)
		// this prevents the duplicate reminder from being sent.
		// $today->setTime(0,0);

		// if($invoice_overdue_date < $today) {
		// 	$this->log->add("MoceanAPI", "Invoice due date is in the past, exiting.");
		// 	return;
		// }
		...
		$sms_reminder = new MoceanSMSReminderDatabase($db);
		...
		// TODO
		// Check if `scheduled datetime` is less than today, if yes, discard
		// today = 1 Jan 2023
		// reminder due date = 24 Jan 2023
		// 5 day before the due date 19 Jan 2023
		// 5 day after the due date 29 Jan 2023

		// then we reinsert them into db
		foreach($sms_reminder_setting as $setting) {
			...
			if($dt->getTimestamp() < $today->getTimestamp()) {
				$this->log->add("MoceanAPI", "Scheduled DT less than Today");
				$this->log->add("MoceanAPI", "Scheduled DT: {$dt->format("Y-m-d H:i:s e")}");
				$this->log->add("MoceanAPI", "Current DT: {$today->format("Y-m-d H:i:s e")}");
				continue;
			}
	...
	{
		global $db;
		$sms_reminder_db = new MoceanSMSReminderDatabase($db);
...
?>
				...
				<form method="POST" action="<?php echo $_SERVER["PHP_SELF"] ?>">
					<table class="border mocean-table">
						...
						<tr>
							...
							<td>
								<table id="sms-reminder-table" class="sms-reminder-table">
		...
		<script>
			...
			const settings = <?php echo json_encode($this->get_settings(), true); ?>;
			const sms_setting = <?php echo json_encode($sms_setting, true) ?>;
			let reminderTableData = settings.reminder_settings;
            ...
            $(document).ready(function() {
				...
				getSMSReminderSkeleton = function( uuid, action="add") {
					...
					let elementToAdd = `
						<h3>${capitalize_first_letter(action)} SMS reminder</h3>
					...
					$('#timepicker').timepicker({
						minTime: sms_setting.MOCEAN_ACTIVE_HOUR_START,
						maxTime: sms_setting.MOCEAN_ACTIVE_HOUR_END,
					});
					const target = msgid
					caretPosition = document.getElementById(target).selectionStart;
            ...
            function populateReminderTable() {
                ...
                */
                ...
                let sms_reminder_table = document.getElementById(`sms-reminder-table`);
