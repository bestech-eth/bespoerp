...
class SMS_Shipment_Setting extends MoceanBaseSettingController
{
	...
	public $db_key = "MOCEANAPI_SHIPMENT_SETTING";
	...
	private function get_default_settings()
	{
		$settings = array(
			...
			"sms_templates" => array(
				"created" 		=> "Hi [company_name], your shipment is created",
				"closed"      	=> "Hi [company_name], your shipment is processed successfully",
	...
	public function update_settings()
	{
		...
		if(!empty($_POST)) {
			...
			if ($action == "update_{$this->context}") {
				...
				if ($error < 1) {
					$this->log->add("MoceanAPI", "failed to update the shipment settings: " . print_r($settings, 1));
	...
	public function trigger_send_sms($object, $status)
	{
		...
		$this->log->add("MoceanAPI", "current status action triggered: {$status}");
