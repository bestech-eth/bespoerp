...
/**
 * \file    test/phpunit/MoceanAPIFunctionalTest.php
 * \ingroup moceanapi
 ...
 *
 * @package Testmoceanapi
 */
class MoceanAPIFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modMoceanAPI')->click();
		} else {
			// Disable the module
			$this->byHref('modMoceanAPI')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/moceanapi/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('moceanapi/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/moceanapi/admin/about.php');
		$this->authenticate();
		return $this->assertContains('moceanapi/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('moceanapiwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modMoceanAPI_MoceanAPITriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modMoceanAPI_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
