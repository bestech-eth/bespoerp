...
$arrayofparameters = array(
	'MOCEANAPI_MYPARAM1'=>array('type'=>'string', 'css'=>'minwidth500' ,'enabled'=>1),
	'MOCEANAPI_MYPARAM2'=>array('type'=>'textarea','enabled'=>1),
	//'MOCEANAPI_MYPARAM3'=>array('type'=>'category:'.Categorie::TYPE_CUSTOMER, 'enabled'=>1),
	//'MOCEANAPI_MYPARAM4'=>array('type'=>'emailtemplate:thirdparty', 'enabled'=>1),
	//'MOCEANAPI_MYPARAM5'=>array('type'=>'yesno', 'enabled'=>1),
	//'MOCEANAPI_MYPARAM5'=>array('type'=>'thirdparty_type', 'enabled'=>1),
	//'MOCEANAPI_MYPARAM6'=>array('type'=>'securekey', 'enabled'=>1),
	//'MOCEANAPI_MYPARAM7'=>array('type'=>'product', 'enabled'=>1),
);
...
if ($useFormSetup && (float) DOL_VERSION >= 15) {
	...
	// Setup conf MOCEANAPI_MYPARAM1 as a simple string input
	$item = $formSetup->newItem('MOCEANAPI_MYPARAM1');

	// Setup conf MOCEANAPI_MYPARAM1 as a simple textarea input but we replace the text of field title
	$item = $formSetup->newItem('MOCEANAPI_MYPARAM2');
	$item->nameText = $item->getNameText().' more html text ';

	// Setup conf MOCEANAPI_MYPARAM3
	$item = $formSetup->newItem('MOCEANAPI_MYPARAM3');
	$item->setAsThirdpartyType();

	// Setup conf MOCEANAPI_MYPARAM4 : exemple of quick define write style
	$formSetup->newItem('MOCEANAPI_MYPARAM4')->setAsYesNo();

	// Setup conf MOCEANAPI_MYPARAM5
	$formSetup->newItem('MOCEANAPI_MYPARAM5')->setAsEmailTemplate('thirdparty');

	// Setup conf MOCEANAPI_MYPARAM6
	$formSetup->newItem('MOCEANAPI_MYPARAM6')->setAsSecureKey()->enabled = 0; // disabled

	// Setup conf MOCEANAPI_MYPARAM7
	$formSetup->newItem('MOCEANAPI_MYPARAM7')->setAsProduct();
	*/
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/moceanapi/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'MOCEANAPI_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'MOCEANAPI_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "moceanapi@moceanapi");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("MoceanapiSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'MOCEANAPI_'.strtoupper($myTmpObjectKey).'_ADDON';
