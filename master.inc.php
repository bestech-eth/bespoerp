 ...
 * 	\ingroup	core
 *  \brief      File that defines environment for all bespoerp process (pages or scripts)
...
$conf->db->host = empty($bespoerp_main_db_host) ? '' : $bespoerp_main_db_host;
$conf->db->port = empty($bespoerp_main_db_port) ? '' : $bespoerp_main_db_port;
$conf->db->name = empty($bespoerp_main_db_name) ? '' : $bespoerp_main_db_name;
$conf->db->user = empty($bespoerp_main_db_user) ? '' : $bespoerp_main_db_user;
$conf->db->pass = empty($bespoerp_main_db_pass) ? '' : $bespoerp_main_db_pass;
$conf->db->type = $bespoerp_main_db_type;
$conf->db->prefix = $bespoerp_main_db_prefix;
...
$conf->file->main_limit_users = $bespoerp_main_limit_users;
$conf->file->mailing_limit_sendbyweb = empty($bespoerp_mailing_limit_sendbyweb) ? 0 : $bespoerp_mailing_limit_sendbyweb;
$conf->file->mailing_limit_sendbycli = empty($bespoerp_mailing_limit_sendbycli) ? 0 : $bespoerp_mailing_limit_sendbycli;
$conf->file->mailing_limit_sendbyday = empty($bespoerp_mailing_limit_sendbyday) ? 0 : $bespoerp_mailing_limit_sendbyday;
$conf->file->main_authentication = empty($bespoerp_main_authentication) ? 'bespoerp' : $bespoerp_main_authentication; // Identification mode
$conf->file->main_force_https = empty($bespoerp_main_force_https) ? '' : $bespoerp_main_force_https; // Force https
$conf->file->strict_mode = empty($bespoerp_strict_mode) ? '' : $bespoerp_strict_mode; // Force php strict mode (for debug)
$conf->file->instance_unique_id = empty($bespoerp_main_instance_unique_id) ? (empty($bespoerp_main_cookie_cryptkey) ? '' : $bespoerp_main_cookie_cryptkey) : $bespoerp_main_instance_unique_id; // Unique id of instance
$conf->file->dol_document_root = array('main' => (string) DOL_DOCUMENT_ROOT); // Define array of document root directories ('/home/htdocs')
$conf->file->dol_url_root = array('main' => (string) DOL_URL_ROOT); // Define array of url root path ('' or '/bespoerp')
if (!empty($bespoerp_main_document_root_alt)) {
	// bespoerp_main_document_root_alt can contains several directories
	$values = preg_split('/[;,]/', $bespoerp_main_document_root_alt);
	$i = 0;
	foreach ($values as $value) {
		$conf->file->dol_document_root['alt'.($i++)] = (string) $value;
	}
	$values = preg_split('/[;,]/', $bespoerp_main_url_root_alt);
	$i = 0;
	foreach ($values as $value) {
		if (preg_match('/^http(s)?:/', $value)) {
			// Show error message
			$correct_value = str_replace($bespoerp_main_url_root, '', $value);
			print '<b>Error:</b><br>'."\n";
			print 'Wrong <b>$bespoerp_main_url_root_alt</b> value in <b>conf.php</b> file.<br>'."\n";
			print 'We now use a relative path to $bespoerp_main_url_root to build alternate URLs.<br>'."\n";
...
if (!defined('NOREQUIREDB')) {
	...
	if ($db->error) {
		...
		if (!defined('USEbespoerpEDITOR') && !defined('USEbespoerpSERVER') && !empty($_SERVER['SCRIPT_FILENAME']) && (strpos($_SERVER['SCRIPT_FILENAME'], DOL_DATA_ROOT.'/website') === 0)) {
...
//unset($bespoerp_main_db_pass); 	// We comment this because this constant is used in some other pages
...
// Create object $mysoc (A thirdparty object that contains properties of companies managed by bespoerp.
