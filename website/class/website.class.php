...
class Website extends CommonObject
{
	...
	public function create(User $user, $notrigger = false)
	{
		...
		if (!$error) {
			$stringtobespoerpfile = "# Some properties for bespoerp web site CMS\n";
			$stringtobespoerpfile .= "param=value\n";
			//print $conf->website->dir_output.'/'.$this->ref.'/.bespoerp';exit;
			file_put_contents($conf->website->dir_output.'/'.$this->ref.'/.bespoerp', $stringtobespoerpfile);
		}
	...
	public function createFromClone($user, $fromid, $newref, $newlang = '')
	{
		...
		$pathofwebsiteold = $bespoerp_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.dol_sanitizeFileName($oldref);
		$pathofwebsitenew = $bespoerp_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.dol_sanitizeFileName($newref);
	...
	public function exportWebSite()
	{
		...
		foreach ($listofpages as $pageid => $objectpageold) {
			...
			$line = '-- File generated by bespoerp '.DOL_VERSION.' -- '.dol_print_date(dol_now('gmt'), 'standard', 'gmt').' UTC --;';
			$line .= "\n";
